...
class FlashRecognizer:
    ...
    def recognize(self, req, data):
        ...
        req_url = self._build_req_with_signature(self.secret_key, query_arr, header)
...
class ASR:
    def __init__(self):
        self.hotword_id = ASRconfig.hotword_id
        self.recognizer=FlashRecognizer(
           appid=ASRconfig.appid,
           secret_id=ASRconfig.secret_id, 
           secret_key=ASRconfig.secret_key,
        )
        self.req = FlashRecognitionRequest(engine_type="16k_zh")


    def request(self, audio_file="./tmp/output.wav"):
        self.req.set_filter_modal(0)
        self.req.set_filter_punc(0)
        self.req.set_filter_dirty(0)
        self.req.set_voice_format("wav")
        self.req.set_word_info(0)
        self.req.set_convert_num_mode(1)
        self.req.set_hotword_id(self.hotword_id)

        with open(audio_file, 'rb') as f:
            data = f.read()
            resultData = self.recognizer.recognize(self.req, data)
            ...
            if len(resp["flash_result"]) == 0:
                print("No ASR result found.")
                return ""
            
            return resp["flash_result"][0]["text"] if resp["flash_result"] else ""

if __name__=="__main__":
    asr = ASR()
    text = asr.request("./tmp/input.wav")
    print(f"识别结果: {text}")
